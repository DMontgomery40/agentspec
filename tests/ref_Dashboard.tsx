 ...
 *     deps:
 *       calls:
 *         - Date(health.timestamp).toLocaleString()}
 *         - Navigate?.
 *         - client.getConfig
 ...
 *
 *     changelog:
 *       - "- 2025-11-01: docs: update CLI_QUICKREF.md with JavaScript/TypeScript support and all flags (f90f540)"
 *     ---/agentspec
...
function Dashboard({ client, onNavigate }: DashboardProps) {
   ...
   *         - useEffect
   *         - useState
   *         - w Date(health.timestamp).toLocaleString()
   *       imports:
   *         - AdminAPIClient
   *         - import AdminAPIClient from '../api/client';
   *         - import type { HealthStatus } from '../api/types';
   ...
   *
   *     changelog:
   *       - "- 2025-09-22: feat(ui): enhance Admin Console with new features and deep linking (b750b40c)"
   *       - "- 2025-09-21: fix: Update Sinch integration and documentation for improved security and functionality (0a33f67c)"
   *       - "- 2025-09-15: v3 plugins prep: manifest get_status + admin refresh endpoint; Diagnostics plugin readiness; Plugins tester UI; Jobs detail refresh; Dashboard plugin note (e499215b)"
   *       - "- 2025-09-15: admin-ui: remove MSW demo, fix base paths, restore mobile drawer + dashboard + plugins; api: mount assets, admin config/settings endpoints + plugins gating; docker: bundle Admin UI; compose: build api from ./api; fix require_api_key order; gitignore: ignore _site/vendor/.bundle (f9ab20e5)"
   *       - "- 2025-09-12: feat(admin-console v2): embedded MCP (SSE/HTTP), persisted settings, inbound UI + verify, logs with tail/search, diagnostics help + deep links, SIP/AMI guidance, Ghostscript required app‑wide; DB status endpoint; docs updates (admin-console, asterisk inbound) (d78a990e)"
   *     ---/agentspec
